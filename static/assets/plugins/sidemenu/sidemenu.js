...
$(window).resize(
    () => {
        ...
        try {
            marginLeftValue = Math.ceil(window.getComputedStyle(menuItems).marginLeft.split('px')[0]);
            marginRightValue = Math.ceil(window.getComputedStyle(menuItems).marginRight.split('px')[0]);
            check = menuItems.scrollWidth + (0 - menuWidth?.offsetWidth) + menuContainerWidth;
        } catch { }
